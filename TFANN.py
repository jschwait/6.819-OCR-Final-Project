'''
File made public on github by Nicholas T. Smith. We slightly modified/trimmed it:
https://github.com/nicholastoddsmith/pythonml/blob/master/TFANN/TFANN.py
...
def _CreateANN(PM, NA, X):
    ...
    for i, NAi in enumerate(NA):
        if NAi[0] == 'AF':      #Apply an activation function ('AF', 'name')
            ...
            print('NAi: {}'.format(NAi))
            print('input (X): {}'.format(X))
            print('filter: {}'.format(W[-1]))
            print('strides: {}'.format(NAi[2]))
...
class ANN:
    ...
    def __init__(self, _IS, _NA, batchSize = None, learnRate = 1e-4, loss = 'l2', maxIter = 1024,
        ...
        '''
        self.IMAGE_SIZE_FOR_CNN = list(_IS)                         #Input shape
    ...
    def CreateModel(self):
        ...
        if self.X is None:  #If no input variable was specified make a new placeholder
            self.X = tf.placeholder("float", [None] + self.IMAGE_SIZE_FOR_CNN)   #Input data matrix of samples
    ...
    def fit(self, A, Y, FD = None):
        ...
        # print(self.X)
        # print(self.Y)
        # print(self.batSz, self.mIter)
        # print(self.loss, self.optmzr)
        # print('Heyyyyy')
        # for o in self.O: print(o)
...
class ANNC(ANN):
    ...
    def __init__(self, _IS, _NA, batchSize = None, learnRate = 1e-3, loss = 'smce', maxIter = 5, 
